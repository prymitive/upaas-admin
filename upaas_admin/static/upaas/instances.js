...
UPAAS.instances_data_from_uwsgi_stats = function (data) {
    var elems = new Array();
    if (data.stats.length > 0) {
        $.each(data.stats, function (i, item) {
            ...
            $.each(item.stats.workers, function (j, worker) {
                ...
                elems.push('<td>' + UPAAS.bytes_to_human(worker.rss) + '</td>');
                ...
                elems.push('<td>' + UPAAS.bytes_to_human(worker.tx) + '</td>');
                elems.push('</tr>');
            });
